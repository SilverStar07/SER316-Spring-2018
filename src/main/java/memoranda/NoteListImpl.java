...
//    public static final String NS_JNNL = "http://www.openmechanics.org/2003/jnotes-noteslist";
    ...
    public Collection getAllNotes() {
        ...
        for (int yi = 0; yi < yrs.size(); yi++) {
            DateInfo.Year y = new DateInfo.Year(yrs.get(yi));
            Vector ms = y.getMonths();
            for (int mi = 0; mi < ms.size(); mi++) {
                DateInfo.Month m = (DateInfo.Month) ms.get(mi);
                Vector ds = m.getDays();
                for (int di = 0; di < ds.size(); di++) {
                    DateInfo.Day d = (DateInfo.Day) ds.get(di);
					Vector ns = d.getNotes();
					for(int ni = 0; ni < ns.size(); ni++) {
						DateInfo.NoteElement n = (DateInfo.NoteElement) ns.get(ni);
        ...
        for (int yi = 0; yi < yrs.size(); yi++) {
            ...
            for (int mi = 0; mi < ms.size(); mi++) {
                ...
                for (int di = 0; di < ds.size(); di++) {
					...
					Vector ns = d.getNotes();
					for(int ni = 0; ni < ns.size(); ni++) {
						DateInfo.NoteElement ne = (DateInfo.NoteElement) ns.get(ni);
    ...
    public INote getNoteForDate(CalendarDate date) {
        DateInfo.Day d = getDay(date);
        if (d == null)
            return null;
		Vector ns = d.getNotes();
		if(ns.size()>0) {
			DateInfo.NoteElement n = (DateInfo.NoteElement) ns.get(0);
    ...
    public INote createNoteForDate(CalendarDate date) {
        DateInfo.Year y = getYear(date.getYear());
        if (y == null)
            y = createYear(date.getYear());
        DateInfo.Month m = y.getMonth(date.getMonth());
        if (m == null)
            m = y.createMonth(date.getMonth());
        DateInfo.Day d = m.getDay(date.getDay());
        if (d == null) 
            d = m.createDay(date.getDay());
		DateInfo.NoteElement ne = d.createNote(Util.generateId());
...
*/
	 public void removeNote(CalendarDate date, String id) {
		...
		for(int i=0;i<ns.size();i++) {
			DateInfo.NoteElement n = (DateInfo.NoteElement) ns.get(i);
...
*/
    private DateInfo.Day getDay(CalendarDate date) {
        DateInfo.Year y = getYear(date.getYear());
        if (y == null)
            return null;
        DateInfo.Month m = y.getMonth(date.getMonth());
  ...
  //TASK 2-2 SMELL BETWEEN CLASSES <Duplicate code with EventsManager.java>
//    private class Year {
