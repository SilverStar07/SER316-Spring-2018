...
/*			_root.addNamespaceDeclaration("jnevents", NS_JNEVENTS);
			...
			if (date.inPeriod(ev.getStartDate(), ev.getEndDate())) {
				if (ev.getRepeat() == REPEAT_DAILY) {
					...
					if ((n - ns) % ev.getPeriod() == 0)
						v.add(ev);
				} else if (ev.getRepeat() == REPEAT_WEEKLY && date.getCalendar().get(Calendar.DAY_OF_WEEK) == ev.getPeriod()) {
					v.add(ev);
				} else if (ev.getRepeat() == REPEAT_MONTHLY && date.getCalendar().get(Calendar.DAY_OF_MONTH) == ev.getPeriod()) {
					v.add(ev);
	...
	private static DateInfo.Day createDay(CalendarDate date) {
	...
	private static DateInfo.Year createYear(int y) {
	...
	private static DateInfo.Year getYear(int y) {
	...
	static DateInfo.Day getDay(CalendarDate date) {
		DateInfo.Year y = getYear(date.getYear());
		if (y == null) {
		    return null;
		}
	...
	//TASK 2-2 SMELL BETWEEN CLASSES <Duplicate code with NoteListImpl.java>
//	static class Year {
